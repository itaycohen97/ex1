    ...
    match event.get("rawPath", "/"):
        case "/entry":
            ...
            if "parkingLot" in args and "plate" in args:
                ...
                parking_lot_db.put_item(
                    Item={
                        "id": return_id,
                        "license_plate": args["plate"],
                        "parking_lot": args["parkingLot"],
                        "enter_time": str(time.time()),
                    }
                )
                return {"statusCode": 200, "body": return_id}
            else:
                return {"statusCode": 200, "body": "missing arguments"}

        case "/exit":
            args = event.get("queryStringParameters", {})
            if args.get("ticketId", None):
                parking_item = parking_lot_db.get_item(
                    Key={"id": args.get("ticketId", {})}
                ).get("Item", None)
                if parking_item:
                    minutes_parked = (
                        time.time() - float(parking_item["enter_time"])
                    ) // 60
                        ...
                        "body": json.dumps(
                            {
                                ...
                                "license_plate": parking_item["license_plate"],
                                "parking_lot": parking_item["parking_lot"],
                            }
                        ),
                    }

                else:
                    return {"statusCode": 200, "body": "Wrong Ticket ID!"}
